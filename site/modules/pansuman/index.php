...
function hastaKaydet() {
    ...
    Redirect('index.php?option=site&bolum=hastalar&task=show&id='.$row->id);
...
function hastaCikar($id) {
    ...
    Redirect("index.php?option=site&bolum=izlemler&task=izlemgetir&tc=".$row->tckimlik);
}
...
function getHastaList($day) {
    ...
    $dbase->setQuery("SELECT COUNT(id) FROM #__hastalar WHERE pasif=0 AND pansuman=1");
    ...
    PansumanHTML::getHastaList($day, $rows, $total);
}