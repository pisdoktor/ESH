...
function NotEkle() {
    ...
    Redirect('index.php?option=site&bolum=hastalar&task=show&id='.$id);
...
function controlTC($tc) {
    ...
    if ($row) {
        ...
        if (tcKimlikKontrol($tc)) {
            ...
            echo '<strong>Girilen Kimlik Numarası Hatalı!!!</strong>';
        }
    }
...
function getHastaList($search, $ilce, $mahalle, $sokak, $kapino, $kayityili, $kayitay, $cinsiyet, $bagimlilik, $ozellik, $ordering) {
   ...
   HastaList::getHastaList($rows, $pageNav, $search, $ilce, $mahalle, $sokak, $kapino, $kayityili, $kayitay, $cinsiyet, $bagimlilik, $ozellik, $ordering);
}
...
function editHasta($id) {
    ...
    */
    ...
    arsort($veri);
    //sık kullanılan 15 hastalığı seçelim
    $veri = array_slice($veri, 0, 15, true);
