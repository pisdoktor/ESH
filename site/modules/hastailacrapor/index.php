...
function saveRapor() {
    ...
    Redirect("index.php?option=site&bolum=hastailacrapor&id=".$hastaid); 
...
function getHastalikList($id) {
    ...
    <div class="row">
    <div class="col-xs-9"><h4><i class="fa-solid fa-file-lines"></i> Hasta İlaç Rapor Bilgileri:  <span style="color:<?php echo $row->cinsiyet == 'E' ? 'blue':'#f5070f';?>"><a href="index.php?option=site&bolum=hastalar&task=show&id=<?php echo $row->id;?>"><?php echo $row->isim." ".$row->soyisim;?></a></span> <sub>(<?php echo $row->anneAdi ? $row->anneAdi:'';?>/<?php echo $row->babaAdi ? $row->babaAdi:'';?>)</sub> <?php echo $row->pasif ? '('.pasifNedeni($row->pasifnedeni).')' : '' ;?></div>
...
<div class="col-sm-6"><?php echo mosHTML::yesnoRadioList('rapor', '', $bilgi[$hastalik->id]->rapor ? '1':'0');?></div>
</div>
...
<div class="col-sm-6"><?php echo mosHTML::yesnoRadioList('raporyeri', '', $bilgi[$hastalik->id]->raporyeri ? '1':'0');?></div>
</div>
